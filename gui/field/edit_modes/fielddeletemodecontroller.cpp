...
void FieldDeleteModeController::onGraphicsItemMousePressed(FieldGraphicsItem *graphics_item, QGraphicsSceneMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::LeftButton) {
        ...
        if (type == FieldGraphicsItemType::WIRE) {
            WireModel *wire = qobject_cast<WireGraphicsItem*>(graphics_item)->getModel<WireModel>();
            int line_index = wire->getGraphicsItem<WireGraphicsItem>()->getLineIndexByCoordinate(event->pos());
            if (line_index != -1) {
