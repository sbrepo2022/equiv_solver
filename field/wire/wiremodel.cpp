...
QList<QLineF> WireGraphicsItem::getTransformedLines() const
{
    ...
    if (this->model != nullptr) lines_list = this->model->getLinesList();
...
{
    QPen pen_lines(this->line_color, this->line_width, Qt::SolidLine, Qt::SquareCap);
...
bool WireModel::mergeOverlappingLines(const QHash<QPoint, int> &lines_endings, const QLine &line1, const QLine &line2,  QLine *line_result = nullptr, bool axis = 0)
{
    ...
    if (line1_s.x1() == line1_s.x2() && line2_s.x1() == line2_s.x2() && line1_s.x1() == line2_s.x1()) {
        ...
        if (line1_s.x2() < line1_s.x1())
            line1_s = QLine(line1_s.p2(), line1_s.p1());

        if (line2_s.x2() < line2_s.x1())
            line2_s = QLine(line2_s.p2(), line2_s.p1());

        if (line2_s.x2() < line1_s.x2()) {
            line_swap = line2_s;
            line2_s = line1_s;
            line1_s = line_swap;
        }
        ...
        if (line2_s.x1() < line1_s.x2()) { // Линии строго накладываются
            if (line2_s.x1() < line1_s.x1())
                line_new = QLine(line2_s.p1(), line2_s.p2());
            else
                line_new = QLine(line1_s.p1(), line2_s.p2());
            is_intersected = true;
        }
        else if (line2_s.x1() == line1_s.x2()) { // Линии строго соприкасаются
            if (lines_endings[line2_s.p1()] == 2) {
                line_new = QLine(line1_s.p1(), line2_s.p2());
                is_intersected = true;
            }
        }
